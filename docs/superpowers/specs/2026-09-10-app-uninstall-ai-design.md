...
struct ResidueGroup: Identifiable {
    ...
    let isSelected: Bool             // 组级勾选（级联）
}

/// 勾选态由面板持有（`selectedResiduePaths: Set<String>`，键 = 残留路径），
/// 不写入 ResidueFile —— 清理管道读取 `entry.residues.filter { 选中 }`。
```
...
- 「卸载」仍提交**全部选中残留 + App 本体**；分组勾选只影响"选中哪些残留"，App 本体默认必删（现有语义）。
