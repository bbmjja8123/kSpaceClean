...
/// Opens the kMonitor dashboard in the main app.
///
...
public struct OpenDashboardIntent: AppIntent {
    ...
    public static var description = IntentDescription(
        "Opens the kMonitor dashboard so you can review current metrics."
    )
    ...
    public func perform() async throws -> some IntentResult {
        ...
        // `kMonitorIntents/OpenDashboardIntent.swift` (Task 21). For the widget
...
struct OpenDashboardButton: View {
    var body: some View {
        Link(destination: URL(string: "kmonitor://open")!) {
