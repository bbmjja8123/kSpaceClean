...
@main
struct kMonitorApp: App {
    @NSApplicationDelegateAdaptor(kMonitorAppDelegate.self) private var appDelegate
    ...
    init() {
        let container = kMonitorAppDelegate.shared.container
        ...
        // surface kMonitor's quick actions. Safe to call on every launch;
        // `KMonitorSpotlightIndexer` deletes the previous index first.
        Task { await KMonitorSpotlightIndexer().reindex() }
        ...
        kMonitorAppDelegate.shared.configureMultiIcon(
    ...
    var body: some Scene {
        ...
        // `widgetURL("kmonitor://open")` (see WidgetViews.swift OpenDashboardButton
        // and ControlWidgetView.swift ControlWidgetView) which the system
        // routes here, bringing kMonitor to the foreground and opening the
        // Dashboard window.
        WindowGroup("kMonitor Deep Link", id: "deeplink") {
            Color.clear
        ...
        Window("kMonitor Dashboard", id: "dashboard") {
        ...
        Window("Welcome to kMonitor", id: "onboarding") {
            OnboardingView(
                viewModel: onboardingViewModel,
                onCloseRequested: {
                    dashboardViewModel.dismissOnboardingBanner()
                    if let window = NSApp.windows.first(where: { $0.title == "Welcome to kMonitor" }) {
                        window.close()
                    }
                }
            )
    ...
    private var appState: AppState { kMonitorAppDelegate.shared.container.appState }
    private var purchaseState: PurchaseState { kMonitorAppDelegate.shared.container.purchaseState }

    /// Route a `kmonitor://` URL to the right in-app destination. Currently
    ...
    private func handleDeepLink(_ url: URL) {
        guard url.scheme?.lowercased() == "kmonitor" else { return }
...
private struct DashboardSceneContent: View {
    ...
    private var dashboardView: some View {
        DashboardView(
            ...
            onOpenPaywall: { showPaywallSheet = true },
            historyRepo: kMonitorAppDelegate.shared.container.historyRepository,
            purchaseState: kMonitorAppDelegate.shared.container.purchaseState,
            processMonitor: kMonitorAppDelegate.shared.container.processMonitor
        )
    }
}
