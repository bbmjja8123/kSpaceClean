...
final class KMonitorPurchaseJourneyTests: XCTestCase {
    ...
    func testFreeUserSeesPaywallCopy() throws {
        ...
        if app.staticTexts["Welcome to kMonitor"].waitForExistence(timeout: 3) {
        ...
        // The paywall sheet contains "kMonitor Pro" and the price line.
        let paywallHeadline = app.staticTexts["kMonitor Pro"]
        XCTAssertTrue(
            paywallHeadline.waitForExistence(timeout: 5),
            "Paywall headline 'kMonitor Pro' did not appear"
        )
