# kMonitor v1.0 — Gap TODO List

> **状态**：进行中（半成品 → 上架就绪）
> **日期**：2026-08-05
> **依据**：kMonitor 与 iStat Menus / iPulse / Stats 的全面 gap 分析（含 2026-08-04 功能维度）
...
| 维度 | 决策 |
|---|---|
| **功能分层** | Free = 全 7 类实时数据（CPU/Memory/Disk/Network/Temperature/Fan/Battery）；Pro = 历史趋势(24h/7d/30d) + 自定义告警阈值 + 进程级网络排行 + Interactive Widget + Live Widgets (WidgetKit TimelineProvider) + 完整 Shortcuts(8个) + 自定义 Widget |
...
| **平台集成** | v1 必做 6 项：基础 Widget + Interactive Widget (macOS 14+) + Live Widgets (WidgetKit TimelineProvider) + 8 Shortcuts + Spotlight + 多图标菜单栏（8 metric 独立位） |
...
- [ ] **I5** 完善 `AppShortcutsProvider`
  - 文件：`kMonitor/Intents/KMonitorAppShortcuts.swift`
...
- [ ] **F1** 菜单栏 popover 暴露全 7 个 metric
  - 文件：`kMonitor/MenuBar/MenuBarView.swift`（重构）
...
- [ ] **F2** Pro 边界在 UI 上清晰呈现
  - 文件：`kMonitor/MenuBar/MenuBarView.swift` + `kMonitor/Dashboard/DashboardView.swift`
  - 温度/风扇/历史 卡片加 🔒 图标 + 灰显
  - 点击 → `PaywallView` sheet
  - 已 Pro 用户正常显示，Pro 徽标在菜单栏右上角

- [ ] **F3** 历史趋势 UI 完整实现
  - 文件：`kMonitor/History/HistoryView.swift`（重构）+ 新增 `HistoryDetailView.swift`
...
- [ ] **F4** 自定义告警阈值 UI
  - 文件：`kMonitor/Alerts/AlertEditorView.swift`（重构）
  - 每个 metric 可配置上下阈值
...
- [ ] **F9** 顶部快捷开关栏
  - 文件：新增 `kMonitor/MenuBar/QuickToggleBar.swift`
  - 4 个 toggle：Wi-Fi / 蓝牙 / 夜览 / Do Not Disturb
  - 调用 AppKit 系统 API（无需 TCC）
  - 失败兜底（Wi-Fi 在沙箱下权限问题）

- [ ] **F10** 菜单栏图标主题化
  - 文件：新增 `kMonitor/MenuBar/MenuBarIconTheme.swift`
...
- [ ] **U9** 多图标菜单栏模式（拖放重排）
  - 文件：`kMonitor/App/kMonitorApp.swift`（重构 Scene）+ `kMonitor/Settings/MenuBarIconLayoutView.swift`（新增）
...
- [ ] **V1** 建立自研 DesignSystem 框架
  ...
  - 集成到 4 个 App（kWise / kSift / kFresh / kMonitor）
...
**Newly added (Stage 1):** T18 Live Widgets (WidgetKit TimelineProvider-based, replaces deprecated Live Activity plan), T19 Shortcuts integration tests, T20 T19 wire fix.
...
- [ ] **K1** 9 kMonitorTests 文件 fail strict-concurrency @ clean HEAD
...
- [x] **K6** Widget extension target 不存在（已修复，见 commit 1206557）
  - ✅ generate_project.py 现在 emit kMonitor (app) / kMonitorIntents (appex) / kMonitorWidget (appex) / kMonitorControlWidget (appex) / kMonitorTests 五个 target
  - ✅ `kMonitor/kMonitorWidget/` + `kMonitor/kMonitorControlWidget/` 编译并嵌入到 `kMonitor.app/Contents/PlugIns/`
  - ⚠️ **kMonitorLiveActivity appex 已删除**（commit eec13c3 — D2/D3 refactor）：ActivityKit 在 Xcode 26 SDK 标注 `@available(macOS, unavailable)`，原 Live Activity 计划替换为 WidgetKit TimelineProvider-based "Live Widgets"，`AppCoordinator` 在告警触发时调用 `WidgetCenter.shared.reloadAllTimelines()`
  - 预估：~150 行 generate_project.py 修改已落地（PBXNativeTarget + embed-in-app + Info.plist + App Group entitlement）
...
- kMonitorIntents appex build：✅（test_files = 0 时也能 build）
- Xcode project 状态：kMonitor target 包含 74 swift 源文件 + 31 test files + 16 appex 源文件
...
### 功能 P1
- [ ] **F5** 进程级网络排行
  - 文件：`kMonitor/Processes/ProcessNetworkRankingView.swift`（新增）
  - 按上传/下载分开排行
...
### UX P1
- [ ] **U3** Dashboard 详情页
  - 文件：`kMonitor/Dashboard/MetricDetailView.swift`（新增）
  - 点击 Dashboard 卡片进入详情
  - 大图 + 历史趋势 + 进程级排行（按 metric 类型）

- [ ] **U4** History 多时间档 + 缩放
  - 文件：`kMonitor/History/HistoryView.swift`（重构）
  - 同 F3（已合并到阶段 1）

- [ ] **U5** 设置面板扩展到 7 tabs
  - 文件：`kMonitor/Settings/SettingsView.swift`（重构）
...
### 集成 P1
- [ ] **I8** Control Widget
  - 文件：`kMonitor/ControlWidget/ControlCenterWidget.swift`（新增）
  - macOS 14+ 控制中心入口
  - 显示当前 CPU/内存使用
  - 点击跳转 Dashboard
...
- [ ] **E8** 国际化字符串集中管理
  - 文件：`kMonitor/Resources/Localizable.xcstrings`
...
> **来源**：`docs/gap-analysis/2026-08-04-kmonitor-vs-top3.md`
...
- [ ] **[gap-2026-08-04] G-U2** 全局快捷键（10+ configurable）
  - 文件：`kMonitor/Settings/HotKeySettingsView.swift`（新增）
...
- [ ] **[gap-2026-08-04] G-E1** CSV/JSON 历史数据导出
  - 文件：`kMonitor/History/HistoryExporter.swift`（新增）
  - 导出格式：CSV（Excel 友好）+ JSON（开发者友好）
  - 时间范围选择：24h / 7d / 30d / Custom
  - 文件保存到 ~/Documents/kMonitor/
...
- [ ] **[gap-2026-08-04] G-E2** 采样间隔可配置（1s-10s per module）
  - 文件：`kMonitor/Settings/SamplingSettingsView.swift`（新增）
...
- [ ] **[gap-2026-08-04] G-I1** AppleScript 字典
  - 文件：`kMonitor/Scripting/`（新增）
  - 支持：`tell application "kMonitor" to get CPU usage`
  - 每 metric 暴露 get/set 命令

- [ ] **[gap-2026-08-04] G-E3** 简化版规则引擎
  - 文件：`kMonitor/Rules/RuleEngine.swift`（新增）
...
| G-E5 | CLI 命令行工具 | Stats E5 | `kmonitor-cli cpu` → 23%，供脚本调用 | 中 |
...
# 沙箱验证：用 kMonitor.entitlements 签名的 .app 经 `open` 启动，读 $HOME 容器内 spike-result.txt
```
